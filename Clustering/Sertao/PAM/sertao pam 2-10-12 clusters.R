...
#setwd("C:\\Users\\Rafaela\\Downloads\\2022\\TFC\\codigos eol\\sertao")
#setwd("/home/rafaela/Documents/PUC/2022.1/TFC/codigos")
getwd()

sertao <- read.csv2("sertao_CE_UCT.csv")
sertao <- sertao[1:359160,]
head(sertao)
tail(sertao)

#sertao$dates_new <- as.POSIXct(sertao$dates_UCT, format = "%Y/%m/%d %H:%M")
#sertao$dates_new <- as.Date(sertao$dates_UCT, format = "%Y/%m/%d %H:%M")
#sertao$dates_new2 <- as.Date(sertao$dates_UCT, format = "%Y/%m/%d")
...
sertao$dates_new <- fernando$dates

maximo = max(sertao$SWGDN)
minimo = min(sertao$SWGDN)
lim <- ceiling(maximo)

sertao$months <- month(sertao$dates_new)
sertao$months1 <- as.character(sertao$months)

head(sertao)

library(dplyr)
sertao_pos <- filter(sertao, SWGDN>0.1)
length(sertao_pos$SWGDN)

PlotsertaoBoxplot <- ggplot(sertao_pos, 
...
for (i in 1:length(sertao$SWGDN))
{sertao$wind_speed_norm[i] = (sertao$SWGDN[i]-minimo)/(maximo-minimo)}

#Plotsertao <- ggplot(sertao) + 
...
min(sertao$wind_speed_norm)

############Data Preparation#######################

vec_norm = as.vector(sertao$wind_speed_norm)
vec = as.vector(sertao$SWGDN)
length(vec)
...
a11 = matrix(0, pam.res$clusinfo[11,1])
aa11 = matrix(0, pam.res$clusinfo[11,1], 24)
ii = 1
for (j in 1:rows) {
  if (pam.res$clustering[j] == 11) {
    a11[ii] = j
    aa11[ii,] = Psi[j,]
    ii = ii + 1
  }
...
a12 = matrix(0, pam.res$clusinfo[12,1])
aa12 = matrix(0, pam.res$clusinfo[12,1], 24)
ii = 1
for (j in 1:rows) {
  if (pam.res$clustering[j] == 12) {
    a12[ii] = j
    aa12[ii,] = Psi[j,]
    ii = ii + 1
  }
...
df
write.csv(df,"porMes_sertao_pam_3K.csv", row.names = FALSE)
write.csv(aa1,"profile1_sertao_pam_3K.csv", row.names = FALSE)
write.csv(aa2,"profile2_sertao_pam_3K.csv", row.names = FALSE)
write.csv(aa3,"profile3_sertao_pam_3K.csv", row.names = FALSE)
write.csv(aa4,"profile4_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa5,"profile5_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa6,"profile6_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa7,"profile7_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa8,"profile8_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa9,"profile9_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa10,"profile10_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa11,"profile11_sertao_pam_12K.csv", row.names = FALSE)
write.csv(aa12,"profile12_sertao_pam_12K.csv", row.names = FALSE)
write.csv(pam.res$clustering, "cluster_sertao_pam_3K.csv", row.names = FALSE)
