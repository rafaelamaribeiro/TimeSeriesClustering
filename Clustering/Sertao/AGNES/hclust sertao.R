...
set.seed(321)
pam.res = agnes(d, method = "ward")
pam.res2 = cutree(pam.res, k=4)
...
for (i in 1:rows) {
  if (months[i] == 1) {
    ...
    else if (pam.res2[[i]] == 2) {
      jan2 = jan2 + 1
    }
    else if (pam.res2[[i]] == 3) {
      jan3 = jan3 + 1
    }
    else if (pam.res2[[i]] == 4) {
      jan4 = jan4 + 1
    }}
  else if (months[i] == 2) {
    if (pam.res2[[i]] == 1) {
...
df
write.csv(df,"porMes_sertao_agnes_4K.csv", row.names = FALSE)
write.csv(pam.res2, "cluster_sertao_agnes_4K.csv", row.names = FALSE)
